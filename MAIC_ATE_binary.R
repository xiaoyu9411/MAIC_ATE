...
theta_AC1_or<-res_AC1_or$lor
V_AC1_or<-res_AC1_or$var

res_BC2_or<-unwt_glm_logOR(t2_B_df$Y, t2_C_df$Y)
theta_BC2_or<-res_BC2_or$lor
V_BC2_or<-res_BC2_or$var

res_BC1_or<-glm_logOR(t2_B_df$Y, t2_C_df$Y, w_B2, w_C2)
theta_BC1_or<-res_BC1_or$lor
V_BC1_or<-res_BC1_or$var

res_AC2_or<-glm_logOR(t1_A_df$Y, t1_C_df$Y, w_A1, w_C1)
theta_AC2_or<-res_AC2_or$lor
V_AC2_or<-res_AC2_or$var
...
logOR_maic_pooled<-w1*(theta_AC1_or-theta_BC1_or)+w2*(theta_AC2_or-theta_BC2_or)
...
Var_maic_or_pooled<-w1^2*(V_AC1_or+V_BC1_or)+w2^2*(V_AC2_or+V_BC2_or)+
                    2*w1*w2*(Cov_AC_or+Cov_BC_or)
SE_maic_or_pooled<-sqrt(Var_maic_or_pooled)
CI_lo_or_pooled<-logOR_maic_pooled-1.96*SE_maic_or_pooled
CI_hi_or_pooled<-logOR_maic_pooled+1.96*SE_maic_or_pooled

logOR_maic_t1<-theta_AC1_or-theta_BC1_or
Var_maic_or_t1<-V_AC1_or+V_BC1_or
SE_maic_or_t1<-sqrt(Var_maic_or_t1)
CI_lo_or_t1<-logOR_maic_t1-1.96*SE_maic_or_t1
CI_hi_or_t1<-logOR_maic_t1+1.96*SE_maic_or_t1

logOR_maic_t2<-theta_AC2_or-theta_BC2_or
Var_maic_or_t2<-V_AC2_or+V_BC2_or
SE_maic_or_t2<-sqrt(Var_maic_or_t2)
CI_lo_or_t2<-logOR_maic_t2-1.96*SE_maic_or_t2
CI_hi_or_t2<-logOR_maic_t2+1.96*SE_maic_or_t2

ATE_un_or<-theta_AC1_or-theta_BC2_or
SE_un_or<-sqrt(V_AC1_or+V_BC2_or)
CI_lo_un_or<-ATE_un_or-1.96*SE_un_or
CI_hi_un_or<-ATE_un_or+1.96*SE_un_or

res_AC1_rr<-unwt_glm_logRR(t1_A_df$Y, t1_C_df$Y)
theta_AC1_rr<-res_AC1_rr$lrr
V_AC1_rr<-res_AC1_rr$var

res_BC2_rr<-unwt_glm_logRR(t2_B_df$Y, t2_C_df$Y)
theta_BC2_rr<-res_BC2_rr$lrr
V_BC2_rr<-res_BC2_rr$var

res_BC1_rr<-glm_logRR(t2_B_df$Y, t2_C_df$Y, w_B2, w_C2)
theta_BC1_rr<-res_BC1_rr$lrr
V_BC1_rr<-res_BC1_rr$var

res_AC2_rr<-glm_logRR(t1_A_df$Y, t1_C_df$Y, w_A1, w_C1)
theta_AC2_rr<-res_AC2_rr$lrr
V_AC2_rr<-res_AC2_rr$var
...
logRR_maic_pooled<-w1*(theta_AC1_rr-theta_BC1_rr)+w2*(theta_AC2_rr-theta_BC2_rr)
...
Var_maic_rr_pooled<-w1^2*(V_AC1_rr+V_BC1_rr)+w2^2*(V_AC2_rr+V_BC2_rr)+
                    2*w1*w2*(Cov_AC_rr+Cov_BC_rr)
SE_maic_rr_pooled<-sqrt(Var_maic_rr_pooled)
CI_lo_rr_pooled<-logRR_maic_pooled-1.96*SE_maic_rr_pooled
CI_hi_rr_pooled<-logRR_maic_pooled+1.96*SE_maic_rr_pooled

logRR_maic_t1<-theta_AC1_rr-theta_BC1_rr
Var_maic_rr_t1<-V_AC1_rr+V_BC1_rr
SE_maic_rr_t1<-sqrt(Var_maic_rr_t1)
CI_lo_rr_t1<-logRR_maic_t1-1.96*SE_maic_rr_t1
CI_hi_rr_t1<-logRR_maic_t1+1.96*SE_maic_rr_t1

logRR_maic_t2<-theta_AC2_rr-theta_BC2_rr
Var_maic_rr_t2<-V_AC2_rr+V_BC2_rr
SE_maic_rr_t2<-sqrt(Var_maic_rr_t2)
CI_lo_rr_t2<-logRR_maic_t2-1.96*SE_maic_rr_t2
CI_hi_rr_t2<-logRR_maic_t2+1.96*SE_maic_rr_t2

ATE_un_rr<-theta_AC1_rr-theta_BC2_rr
SE_un_rr<-sqrt(V_AC1_rr+V_BC2_rr)
CI_lo_un_rr<-ATE_un_rr-1.96*SE_un_rr
CI_hi_un_rr<-ATE_un_rr+1.96*SE_un_rr

res_AC1_rd<-unwt_glm_RD(t1_A_df$Y, t1_C_df$Y)
theta_AC1_rd<-res_AC1_rd$rd
V_AC1_rd<-res_AC1_rd$var

res_BC2_rd<-unwt_glm_RD(t2_B_df$Y, t2_C_df$Y)
theta_BC2_rd<-res_BC2_rd$rd
V_BC2_rd<-res_BC2_rd$var

res_BC1_rd<-glm_RD(t2_B_df$Y, t2_C_df$Y, w_B2, w_C2)
theta_BC1_rd<-res_BC1_rd$rd
V_BC1_rd<-res_BC1_rd$var

res_AC2_rd<-glm_RD(t1_A_df$Y, t1_C_df$Y, w_A1, w_C1)
theta_AC2_rd<-res_AC2_rd$rd
V_AC2_rd<-res_AC2_rd$var

RD_maic_pooled<-w1*(theta_AC1_rd-theta_BC1_rd)+w2*(theta_AC2_rd-theta_BC2_rd)

Cov_AC_rd<-V_AC1_rd
Cov_BC_rd<-V_BC2_rd

Var_maic_rd_pooled<-w1^2*(V_AC1_rd+V_BC1_rd)+w2^2*(V_AC2_rd+V_BC2_rd)+
                    2*w1*w2*(Cov_AC_rd+Cov_BC_rd)
SE_maic_rd_pooled<-sqrt(Var_maic_rd_pooled)
CI_lo_rd_pooled<-RD_maic_pooled-1.96*SE_maic_rd_pooled
CI_hi_rd_pooled<-RD_maic_pooled+1.96*SE_maic_rd_pooled

RD_maic_t1<-theta_AC1_rd-theta_BC1_rd
Var_maic_rd_t1<-V_AC1_rd+V_BC1_rd
SE_maic_rd_t1<-sqrt(Var_maic_rd_t1)
CI_lo_rd_t1<-RD_maic_t1-1.96*SE_maic_rd_t1
CI_hi_rd_t1<-RD_maic_t1+1.96*SE_maic_rd_t1

RD_maic_t2<-theta_AC2_rd-theta_BC2_rd
Var_maic_rd_t2<-V_AC2_rd+V_BC2_rd
SE_maic_rd_t2<-sqrt(Var_maic_rd_t2)
CI_lo_rd_t2<-RD_maic_t2-1.96*SE_maic_rd_t2
CI_hi_rd_t2<-RD_maic_t2+1.96*SE_maic_rd_t2

ATE_un_rd<-theta_AC1_rd-theta_BC2_rd
SE_un_rd<-sqrt(V_AC1_rd+V_BC2_rd)
CI_lo_un_rd<-ATE_un_rd-1.96*SE_un_rd
CI_hi_un_rd<-ATE_un_rd+1.96*SE_un_rd
...
cat("=================================================================================\n")
cat(sprintf("%-26s %-8s %8s %8s %26s\n", "Method", "TE", "Estimate", "SE", "95% CI"))
cat("---------------------------------------------------------------------------------\n")
cat(sprintf("%-26s %-8s %8.4f %8.4f  [%7.4f, %7.4f]\n",
            "MAIC pooled",     "log OR", logOR_maic_pooled, SE_maic_or_pooled, CI_lo_or_pooled, CI_hi_or_pooled))
cat(sprintf("%-26s %-8s %8.4f %8.4f  [%7.4f, %7.4f]\n",
            "MAIC Trial 1 pop","log OR", logOR_maic_t1,     SE_maic_or_t1,     CI_lo_or_t1,     CI_hi_or_t1))
cat(sprintf("%-26s %-8s %8.4f %8.4f  [%7.4f, %7.4f]\n",
            "MAIC Trial 2 pop","log OR", logOR_maic_t2,     SE_maic_or_t2,     CI_lo_or_t2,     CI_hi_or_t2))
cat(sprintf("%-26s %-8s %8.4f %8.4f  [%7.4f, %7.4f]\n",
            "IPTW pooled IPD", "log OR", logOR_ipw,         SE_ipw_or,         CI_lo_ipw_or,    CI_hi_ipw_or))
cat(sprintf("%-26s %-8s %8.4f %8.4f  [%7.4f, %7.4f]\n",
            "Unadjusted",      "log OR", ATE_un_or,         SE_un_or,          CI_lo_un_or,     CI_hi_un_or))

cat("---------------------------------------------------------------------------------\n")
cat(sprintf("%-26s %-8s %8.4f %8.4f  [%7.4f, %7.4f]\n",
            "MAIC pooled",     "log RR", logRR_maic_pooled, SE_maic_rr_pooled, CI_lo_rr_pooled, CI_hi_rr_pooled))
cat(sprintf("%-26s %-8s %8.4f %8.4f  [%7.4f, %7.4f]\n",
            "MAIC Trial 1 pop","log RR", logRR_maic_t1,     SE_maic_rr_t1,     CI_lo_rr_t1,     CI_hi_rr_t1))
cat(sprintf("%-26s %-8s %8.4f %8.4f  [%7.4f, %7.4f]\n",
            "MAIC Trial 2 pop","log RR", logRR_maic_t2,     SE_maic_rr_t2,     CI_lo_rr_t2,     CI_hi_rr_t2))
cat(sprintf("%-26s %-8s %8.4f %8.4f  [%7.4f, %7.4f]\n",
            "IPTW pooled IPD", "log RR", logRR_ipw,         SE_ipw_rr,         CI_lo_ipw_rr,    CI_hi_ipw_rr))
cat(sprintf("%-26s %-8s %8.4f %8.4f  [%7.4f, %7.4f]\n",
            "Unadjusted",      "log RR", ATE_un_rr,         SE_un_rr,          CI_lo_un_rr,     CI_hi_un_rr))

cat("---------------------------------------------------------------------------------\n")
cat(sprintf("%-26s %-8s %8.4f %8.4f  [%7.4f, %7.4f]\n",
            "MAIC pooled",     "RD",     RD_maic_pooled,    SE_maic_rd_pooled, CI_lo_rd_pooled, CI_hi_rd_pooled))
cat(sprintf("%-26s %-8s %8.4f %8.4f  [%7.4f, %7.4f]\n",
            "MAIC Trial 1 pop","RD",     RD_maic_t1,        SE_maic_rd_t1,     CI_lo_rd_t1,     CI_hi_rd_t1))
cat(sprintf("%-26s %-8s %8.4f %8.4f  [%7.4f, %7.4f]\n",
            "MAIC Trial 2 pop","RD",     RD_maic_t2,        SE_maic_rd_t2,     CI_lo_rd_t2,     CI_hi_rd_t2))
cat(sprintf("%-26s %-8s %8.4f %8.4f  [%7.4f, %7.4f]\n",
            "IPTW pooled IPD", "RD",     RD_ipw,            SE_ipw_rd,         CI_lo_ipw_rd,    CI_hi_ipw_rd))
cat(sprintf("%-26s %-8s %8.4f %8.4f  [%7.4f, %7.4f]\n",
            "Unadjusted",      "RD",     ATE_un_rd,         SE_un_rd,          CI_lo_un_rd,     CI_hi_un_rd))
