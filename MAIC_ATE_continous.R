...
delta_AC1 <- res_AC1$delta;  V_AC1 <- res_AC1$var

res_BC2   <- unwt_lm_eff(t2_B_df$Y, t2_C_df$Y)
delta_BC2 <- res_BC2$delta;  V_BC2 <- res_BC2$var

res_BC1   <- wt_lm_eff(t2_B_df$Y, t2_C_df$Y, w_B2, w_C2)
delta_BC1 <- res_BC1$delta;  V_BC1 <- res_BC1$var

res_AC2   <- wt_lm_eff(t1_A_df$Y, t1_C_df$Y, w_A1, w_C1)
delta_AC2 <- res_AC2$delta;  V_AC2 <- res_AC2$var
...
# Pooled ATE and variance

N  <- nrow(trial1) + nrow(trial2)
w1 <- nrow(trial1) / N
w2 <- nrow(trial2) / N

ATE_maic <- w1*(delta_AC1 - delta_BC1) + w2*(delta_AC2 - delta_BC2)

Var_maic <- w1^2*(V_AC1 + V_BC1) + w2^2*(V_BC2 + V_AC2) +
            2*w1*w2*(V_AC1 + V_BC2)

SE_maic  <- sqrt(Var_maic)
CI_lo    <- ATE_maic - 1.96*SE_maic
CI_hi    <- ATE_maic + 1.96*SE_maic
...
cat("----------------------------------------------------------\n")
cat(sprintf("%-20s %8.4f %8.4f  [%7.4f, %7.4f]\n",
            "MAIC-based", ATE_maic, SE_maic, CI_lo,    CI_hi))
cat(sprintf("%-20s %8.4f %8.4f  [%7.4f, %7.4f]\n",
            "IPW-based",  ATE_ipw,  SE_ipw,  CI_lo_ipw, CI_hi_ipw))
